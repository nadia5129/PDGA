...
async function buildEventCharts(options = {}) {
  ...
  try {
    ...
    // Determine years from matched events/results but include only years that have data
    ...
    // Filter to years that actually have data (cash > 0, players > 0, or score rows)
    const years = sortedCandidates.filter(y => {
      const cashVal = perYearCash[y] || 0;
      const playersVal = perYearPlayers[y] ? perYearPlayers[y].size : 0;
      const scoreCnt = perYearCount[y] || 0;
      return (cashVal > 0) || (playersVal > 0) || (scoreCnt > 0);
    });

    if (years.length === 0) {
      console.info('buildEventCharts: no years with data for', title, ' — nothing to plot');
